...
import { CreateReactContextRenderMiddleware, DefaultTsxRenderMiddleware, PrettifyRenderMiddleware, ReactViewsContext, TsxRenderContext } from 'express-tsx-views';
...
export class JsxInterceptor<T> implements NestInterceptor<T, Response<T>> {
  intercept(context: ExecutionContext, next: CallHandler): Observable<Response<T>> {
    ...
    return next.handle().pipe(mergeMap(async (data: any) => {
